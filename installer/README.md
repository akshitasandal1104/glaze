...
## How deployment code works (terraform wrapped in lambda):

Structure:
  settings/common.py - > contains the order and what is deployed
  core/aws -> all python classes wrapping the aws api calls needed for logic and getting thing that terraform can't
  core/terraform -> all the python class wrapping the terraform base code

How to add a resource.. you need to add the following:
1. terraform python wrapper class in core/terraform dir
2. add any aws needed calls in core/aws
3. add neew resource to PROCESS_RESOURCES dict in common.py
